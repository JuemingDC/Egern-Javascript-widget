      ...
      children: [
        {
          ...
          children: [
            ...
            },
            {
              type: "text",
              text: "订阅异常",
              font: { size: density.leftSub, weight: "medium", family: "Menlo" },
              textColor: theme.warn,
              maxLines: 1,
            },
          ],
        },
        {
          ...
          children: [
            ...
            buildRail(0.08, theme.warn, theme, density, true),
            {
              type: "text",
    ...
    children: [
      {
        ...
        children: [
          ...
          },
          {
            type: "text",
            text: stage.label,
            font: { size: density.leftSub, weight: "semibold", family: "Menlo" },
...
function buildRail(fillRatio, color, theme, density, isError) {
  const fill = Math.max(0.08, Math.min(fillRatio, 1));
  const rest = Math.max(0.06, 1 - fill);
  return {
    type: "stack",
    direction: "row",
    alignItems: "center",
...
function getStage(remainingPercent, theme) {
  if (remainingPercent >= 65) return { label: "充裕", color: theme.ok };
  if (remainingPercent >= 35) return { label: "平衡", color: theme.cool };
  if (remainingPercent >= 15) return { label: "紧张", color: theme.warm };
  return { label: "告急", color: theme.danger };
}
